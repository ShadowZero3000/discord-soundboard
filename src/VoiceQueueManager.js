...
class PrivateVoiceQueueManager {
  ...
  getQueueFromChannel (voiceChannel) {
    ...
    if (!this.queues[voiceChannel.id]) {
      ...
      log.info(`New voice queue created: ${this.queues[voiceChannel.id]} - ${voiceChannel.id}`)
    }
    // log.debug(`Voice queue: ${this.queues[voiceChannel.id]}`)
    return this.queues[voiceChannel.id]
  }

  async getQueueFromUser (userId) {
    const voiceChannel = await this.getVCFromUserid(userId)
    ...
    log.info(`Found queue for user: ${res}`)
    return res
  }

  getQueueFromMessage (message) {
    log.debug(`Looking for an active voice channel for ${message.member.user.id} in qFromMessage`)
  ...
  async getVCFromUserid (userId) {
    const discord = DiscordBot.getInstance().client
    log.debug(`Looking for an active voice channel for ${userId} in getVCFromUserid`)
    const vsPromises = discord.guilds.cache.map(async (guild) => {
      //log.debug(`Given guild for finding voicestates: ${guild}. ${guild.voiceStates}`)
      const result = await guild.voiceStates.fetch(userId).catch((err) => {
        //log.debug(`guildVS Fetch fail: ${err}`)
        return undefined
      })
      return result
    })

    const voiceStates = await Promise.all(vsPromises)
    const voiceChannels =
      voiceStates
