import { SlashCommandBuilder, ModalBuilder, TextInputStyle, TextInputBuilder, ActionRowBuilder, SlashCommandAttachmentOption, MessageFlags } from 'discord.js'
...
async function request (interaction) {
  ...
  if (!clip.match(/^[a-z0-9_]+$/)) {
    return await interaction.reply({ content: `${clip} is a bad clip name`, flags: MessageFlags.Ephemeral })
  }
  if (fm.addRequest(clip, description)) {
    return await interaction.reply({ content: 'Ok, I\'ll add it to the list', flags: MessageFlags.Ephemeral })
  } else {
    return await interaction.reply({ content: 'Already on the list', flags: MessageFlags.Ephemeral })
  }
}
...
async function move (interaction) {
  ...
  if (!category.match(/^[a-z0-9_]+$/)) {
    return await interaction.reply({ content: `${category} is a bad category name`, flags: MessageFlags.Ephemeral })
  }
  if (!subcategory.match(/^[a-z0-9_]+$/)) {
    return await interaction.reply({ content: `${subcategory} is a bad subcategory name`, flags: MessageFlags.Ephemeral })
  }

  if (fm.inLibrary(clip)) {
    fm.rename(clip, clip, category, subcategory)
    return await interaction.reply({ content: `'${clip}'s category is now: ${category} - ${subcategory}`, flags: MessageFlags.Ephemeral })
  } else {
    return await interaction.reply({ content: `I don't recognize '${clip}'`, flags: MessageFlags.Ephemeral })
  }
}
...
async function rename (interaction) {
  ...
  if (!newName.match(/^[a-z0-9_]+$/)) {
    return await interaction.reply({ content: `'${newName}' is a bad clip name`, flags: MessageFlags.Ephemeral })
  }

  if (fm.inLibrary(clip)) {
    fm.rename(clip, newName)
    return await interaction.reply({ content: `'${clip}' has been renamed to '${newName}'`, flags: MessageFlags.Ephemeral })
  } else {
    return await interaction.reply({ content: `I don't recognize ${clip}`, flags: MessageFlags.Ephemeral })
  }
}
...
async function remove (interaction) {
  ...
  if (!fm.inLibrary(clipName)) {
    return await interaction.reply({ content: `'${clipName}' not found. Did you typo it?`, flags: MessageFlags.Ephemeral })
  }
  if (fm.delete(clipName)) {
    return await interaction.reply({ content: `'${clipName}' removed.`, flags: MessageFlags.Ephemeral })
  }
}
...
export async function handleModal (modalSubmission) {
  ...
  const upload = Object.assign({}, pendingUploads[uuid]) // So we can delete the hash ref
